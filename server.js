...
function serveFile(res, pathname){
  if(pathname === "/jackspot" || pathname === "/jackspot/"){
    res.writeHead(302, {"Location":"/jackspot.html"});
    res.end();
    return;
  }
...
const server = http.createServer(async (req, res)=>{
  ...
  try{
    ...
    if(url.pathname === "/api/sessions/start" && req.method === "POST"){
      ...
      const iframeUrlFor = sessionId=>`/jackspot.html?machine=${encodeURIComponent(machine.machineId)}&server=${encodeURIComponent(url.origin)}&creditBaseline=0&playSessionId=${encodeURIComponent(String(sessionId || ""))}`;
